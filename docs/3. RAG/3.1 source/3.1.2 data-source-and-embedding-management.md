...
## Source Structure and Parsing Strategy

Different source files may require different parsing logic. The ingestion pipeline should not assume that every file can be converted to text in the same way.

File format is only a first-level signal. It should not be the only parser selection rule. Two files can both be PDFs but have completely different internal structures and require different parsing strategies.

Examples:

```text
pdf / policy document         -> PDF text parser with section heading detection
pdf / scanned contract        -> OCR-aware parser
pdf / financial report        -> table-aware parser
docx / product manual         -> document parser with heading hierarchy
html / support article        -> HTML cleaner and text extractor
csv / product catalog         -> tabular parser
json / API reference          -> structured JSON parser
```
...
source_type = "policy"
structure_type = "sectioned-document"
parser_profile = "pdf-sectioned-policy"
```

Another PDF source might use a different profile:

```text
source_path = "knowledge_base/sources/reports/q4-financial-report.pdf"
source_format = "pdf"
mime_type = "application/pdf"
source_type = "financial-report"
structure_type = "tables-and-narrative"
parser_profile = "pdf-table-aware-report"
```
