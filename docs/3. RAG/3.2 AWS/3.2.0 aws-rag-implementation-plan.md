...
### Model profile configuration in IaC

`model-profiles.json` is the environment-level model configuration source managed alongside IaC.

It should describe the selected RAG model components for each environment, for example:
...
This lets `dev`, `staging`, and `prod` use different model combinations without changing application business logic.

In the IaC flow, `model-profiles.json` can be used for three purposes:

1. Provide non-secret runtime configuration to the deployed application.
2. Drive vector index naming and compatibility checks, because each vector index is tied to an embedding provider, model, and dimension.
3. Seed a persistent runtime registry such as DynamoDB `rag_model_profiles` when the system moves beyond file-based configuration.

Do not store secret values in `model-profiles.json`.

Store only secret references, for example:
...
The actual API key or credential value should live in AWS Secrets Manager.

Changing only the generation LLM usually does not require rebuilding the vector index, but evaluation should be rerun.
